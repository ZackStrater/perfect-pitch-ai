...
def save_and_split(path_in, midi_out, audio_out, midi_win_out='', save_midi_windows=False, n_mels=128, stepsize=30, left_buffer=50, right_buffer=19, apply_sus=True):
    ...
    for midi_file, audio_file in zip(midi_bin, audio_bin):
        ...
        try:
            song.process_audio_midi_save_slices(midi_out, audio_out, normalize_mel_spectrogram=True, n_mels=n_mels, apply_sus=apply_sus,
                                                stepsize=stepsize, left_buffer=left_buffer, right_buffer=right_buffer, filename=midi_filename, file_format='png',
